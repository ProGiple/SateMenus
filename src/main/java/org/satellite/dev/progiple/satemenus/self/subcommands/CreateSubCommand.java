...
@SubCommand(appliedCommand = "satemenus", commandIdentifiers = {"create"})
@Permissions("#.create")
@Flags(NoArgCommand.AccessFlag.PLAYER_ONLY)
@Args(min = 3, max = Integer.MAX_VALUE)
public class CreateSubCommand implements LunaExecutor {
    @Override
    public void invoke(CommandSender sender, String[] strings) {
        ...
        var builder = Creator.startCreating(strings[2], generatorType, player.getUniqueId());
        new MainEditMenu(player, builder).open();
    }

    @Override
    public List<String> tabComplete(CommandSender sender, List<String> list) {
        return list.size() == 1 ? List.of("menu", "template") : list.size() == 2 ? List.of("<id>") : null;
    }
}
